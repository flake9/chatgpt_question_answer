...
def requests_retry_session(retries=2, backoff_factor=0.5, status_forcelist=(500, 502, 504)):
...
class Chatbot:
    ...
    def __init__(
        ...
        user_home = getenv("HOME") or Path.home()
        ...
        self.config = config
        self.session = session_client() if session_client else requests_retry_session()
        try:
    ...
    def ask(
        ...
        data = {
            ...
            "model": "text-davinci-002-render-sha" if not self.config.get("paid") else "text-davinci-002-render-paid",
        }
        ...
        response = self.session.post(
            url=BASE_URL + "api/conversation", data=json.dumps(data), timeout=timeout, stream=True
        )
        ...
        for line in response.iter_lines():
            ...
            if not self.__check_fields(line):
                ...
                if line.get("detail") == "Too many requests in 1 hour. Try again later.":
    ...
    async def ask(
        ...
        async with self.session.stream(
            ...
            async for line in response.aiter_lines():
                ...
                parent_id = line["message"]["id"]
                model = (
                    line["message"]["metadata"]["model_slug"] if "model_slug" in line["message"]["metadata"] else None
                )
                yield {
                    "message": message,
                    "conversation_id": conversation_id,
